...
class Robot(object):
    def __init__(self, addr=0x60, left_id=3, right_id=1, left_trim=0, right_trim=0,
                 stop_at_exit=True):
...
#Tasta alkaa omat lisaykset
    def RT_forward(self, speed, seconds=None):
	self._right_speed(speed)
	self._right.run(Adafruit_MotorHAT.FORWARD)

    def RT_backward(self, speed, seconds=None):
	self._right_speed(speed)
	self._right.run(Adafruit_MotorHAT.BACKWARD)

    def LT_forward(self, speed, seconds=None):
        self._left_speed(speed)
        self._left.run(Adafruit_MotorHAT.FORWARD)

    def LT_backward(self, speed, seconds=None):
        self._left_speed(speed)
